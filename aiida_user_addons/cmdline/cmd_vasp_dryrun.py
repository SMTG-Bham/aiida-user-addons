...
@click.option(
    "--vasp-exe", help="Executable for VASP", default="vasp_std", show_default=True
)
@click.option(
    "--timeout",
...
@click.option(
    "--keep", help="Wether to the dryrun files", is_flag=True, show_default=True
)
@click.option(
    "--force",
...
def vasp_dryrun(
    input_dir, vasp_exe="vasp_std", timeout=10, work_dir=None, keep=False, force=False
):
    """
        ...
        if work_dir.exists():
            if not force:
                raise FileExistsError(
                    f"Working directory {work_dir} exists already! Please remove it first."
                )
            else:
...
def parse_outcar(outcar_path):
    ...
    for il, line in enumerate(lines):
        if "POTCAR:" in line:
            ...
            k_list = [
                [float(token) for token in subline.strip().split()]
