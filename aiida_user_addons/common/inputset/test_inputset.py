...
def test_castep(fe_atoms, mgo_atoms):
    ...
    iset = CASTEPInputSet('UCLCASTEPRelaxSet', fe_atoms, overrides={'spin_list': [1], 'hubbardu_mapping': {'Fe': ['d', 5.0]}})
    out = iset.get_input_dict()
    ...
    iset = CASTEPInputSet('UCLCASTEPRelaxSet', fe_atoms, overrides={'spin_mapping': {'Fe': 4.0}, 'hubbardu_mapping': {'Fe': ['d', 5.0]}})
    out = iset.get_input_dict()
    settings = iset.get_settings()
    assert out['hubbard_u'] == ['Fe  d:5.0']
    ...
    iset = CASTEPInputSet('UCLCASTEPRelaxSet', mgo_atoms, overrides={'spin_mapping': {'Mg': 2.0}})
