...
Implement for CASTEP using forces parsed from calculations
"""
import numpy as np
...
from aiida_user_addons.common.opthold import IntOption, ListOption, OptionContainer, OptionHolder, ListOrStringOption, FloatOption
...
class CastepAutoPhononWorkChain(WorkChain):
    """
    CastepAutoPhononWorkChain
    ...
    """
    _relax_entrypoint = 'castep.relax'
    _relax_chain = WorkflowFactory(_relax_entrypoint)
    _singlepoint_entrypoint = 'castep.base'
    ...
    def define(cls, spec):
        ...
        spec.outline(
            ...
            if_(cls.should_run_supercell_charge_density)(
                cls.run_supercell_castepbin,
    ...
    def create_displacements(self):
        ...
        if self.should_run_relax():
            ...
            try:
                # Inspect the inputs of the relaxation calculation
                magmom = relax_calc_inputs.calc.settings.get('SPINS')
    ...
    def run_supercell_castepbin(self):
        """
        Run a supercell calculation to boot strap the calculations
        using the charge density from the CASTEP bin file

        Should check if this works as intended - unlike VASP, CASTEP default to do a
        band structure calculation to restore the full wavefunction.
        """

        calc_inputs = self.exposed_inputs(self._singlepoint_chain, 'singlepoint')
        calc_inputs.calc.structure = self.ctx.supercell

        # For magnetisation
        magmom = self.ctx.phonon_setting_info.get_dict().get('_supercell_magmom')
        if magmom:
            self.report('Using MAGMOM from the phonopy output')
            settings = calc_inputs.calc.settings.get_dict()
            settings['SPINS'] = magmom
            calc_inputs.calc.settings = orm.Dict(dict=settings)

        # TODO: Ensure that the castepbin file is written
    ...
    def run_force_and_nac_calcs(self):
        ...
        if self.should_run_supercell_charge_density():
            # Set start from constant charge
            force_calc_inputs.reuse_folder = self.ctx.supercell_remote_folder

        magmom = self.ctx.phonon_setting_info.get_dict().get('_supercell_magmom')
        if magmom:
            self.report('Using MAGMOM from the phonopy output')
            settings = force_calc_inputs.calc.settings.get_dict()
            settings['SPINS'] = magmom
            force_calc_inputs.calc.settigns.parameters = orm.Dict(dict=settings)
        ...
        if self.is_nac():
            raise NotImplemented('NAC Has not been implemented')
            ...
            # nac_inputs.calc.structure = self.ctx.primitive
    ...
    def create_force_set_and_constants(self):
        ...
        TODO: Here I need to adapt it for CASTEP.
        """
    ...
    def is_nac(self):
        ...
        Returns True if the 'nac' input namespace exists.

        CASTEP DFPT needs to use NCPs and cannot include the Hubbard U correction,
    ...
    def remote_phonopy(self):
        """Wether to run phonopy as a remote code"""
...
def collect_castep_forces_and_energies(ctx, ctx_supercells, prefix='force_calc', obj=None):
    ...
    for key in ctx_supercells:
        ...
        if type(calc) is dict:
            ...
            calc_dict = calc.outputs
        if 'output_array' not in calc_dict:
            msg = 'Force not found in the CastepBaseWorkChain - trying to recover using the last called VaspCalculation - procedd with caution.'
            if obj:
                obj.report(msg)
            else:
                print(msg)
        ...
        if ('output_array' in calc_dict and 'forces' in calc_dict['output_array'].get_arraynames()):
            forces_dict['forces_{}'.format(num)] = calc_dict['output_array']
        else:
            raise RuntimeError('Forces could not be found in calculation {}.'.format(num))

        if ('output_parameters' in calc_dict and
                'zero_K_energy' in calc_dict['output_parameters'].keys()):  # needs .keys() - calc_dict can be a dict or a LinkManager
            forces_dict['misc_{}'.format(num)] = calc_dict['output_parameters']
...
@calcfunction
def get_castep_force_sets_dict(**forces_dict):
    forces = []
    energies = []
    forces_0 = None
    energy_0 = None

    # Construct the place holder array
    ...
    for key in forces_dict:
        ...
        if 'forces' in key:
            ...
            # Use the 0 K extraplated energy if avaliable, otherwise try others
            # Calculations with fixed occupation may only have the total energy
            for energy_key in ('zero_K_energy', 'free_energy', 'total_energy'):
                energy = forces_dict[key][energy_key]
                if num == 0:
                    energy_0 = energy
                else:
