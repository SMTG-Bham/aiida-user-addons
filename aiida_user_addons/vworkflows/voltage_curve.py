...
from aiida_user_addons.process.battery import (
    ...
    create_delithiated_multiple_level,
)
...
class EwaldFilterOptions(OptionContainer):
    ...
    pick_ewald_n_lowest = IntOption(
        "Number of lowest energy structures to be picked", default_value=10
    )
...
class VoltageCurveWorkChain(WorkChain, WithVaspInputSet):
    ...
    def define(cls, spec):
        ...
        spec.input(
            ...
            valid_type=orm.Dict,
            required=False,
            help="The misc output of a calculation for the fully lithiated phase. If passed, will not perform any relaxation for the initial lithiated structure.",
        )
        spec.input(
            "li_metal_calc_misc",
            valid_type=orm.Dict,
        ...
        spec.input(
            ...
            required=False,
            valid_type=orm.Float,
            help="Symmetry tolerance for generating unique structures.",
        )
        spec.input(
            "ldau_mapping",
    ...
    felec: Wether we are dealing with f electrons, will increase lmaxmix if we are.""",
        ...
        spec.output(
            ...
            valid_type=orm.Dict,
            required=True,
            help="Summary data regarding the voltage curve.",
        )
    ...
    def verify_initial_relax(self):
        ...
        try:
            ...
            self.ctx.current_structure = (
                workchain.outputs.relax.structure
            )  # WORKCHAIN specific!
    ...
    def generate_delithiated_structures(self):
        ...
        delithiated = create_delithiated_multiple_level(
            structure, self.inputs.final_li_level, self.inputs.rattle, **params
        )
    ...
    def result(self):
        ...
        record_node = compose_voltage_curve_data(
            self.inputs.li_metal_calc_misc, **miscs
        )
...
def get_voltage_curve_obj(workchain):
    ...
    if "lithiated_calc_misc" in workchain.inputs:
        ...
        lithiated_calc = get_returner(
            workchain.outputs.relaxed_lithiated_structure, VaspRelaxWorkChain
        )
