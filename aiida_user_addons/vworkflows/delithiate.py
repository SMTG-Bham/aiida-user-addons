...
from aiida_user_addons.process.battery import (
    DelithiationManager,
    check_li_ref_calc,
    compute_li_voltage_shortcut,
)
from aiida_user_addons.process.transform import (
    delithiate_by_wyckoff,
    delithiate_full,
    delithiate_unique_sites,
    rattle,
)
...
class SimpleDelithiateWorkChain(WorkChain, WithVaspInputSet):
    ...
    def define(cls, spec):
        ...
        spec.input(
            "strategy",
            valid_type=orm.Str,
            help="Delithiation strategy to be used. Choose from: full, unique, wyckoff.",
        )
        spec.input(
            "rattle_amp",
        ...
        spec.input(
            ...
            valid_type=orm.List,
            required=False,
            help="Wyckoff sites to remove for the wyckoff mode.",
        )
        spec.input_namespace(
            "unique_options",
            required=False,
            populate_defaults=False,
            help="Options for delithiate by unique sites",
        )
        spec.input(
            "unique_options.excluded_sites",
            required=False,
            default=lambda: orm.List(list=[]),
            help="Excluded site indices.",
        )
        spec.input(
            "unique_options.nsub",
            required=False,
            default=lambda: orm.Int(1),
            help="Number of Li to remove",
        )
        spec.input(
            "unique_options.atol",
            required=False,
        ...
        spec.input(
            "unique_options.limit",
            required=False,
            help="Maximum number of structures to attempt",
        )
        spec.input(
            "ldau_mapping",
        ...
        spec.input(
            "deli_magmom_mapping",
            required=True,
            help="Mapping for MAGMOM for the delithiated state",
            valid_type=orm.Dict,
        )
        spec.outline(
            cls.setup,
        ...
        spec.output_namespace(
            "relaxed_delithiated_structures",
            dynamic=True,
            valid_type=orm.StructureData,
            required=True,
        ...
        spec.output(
            "relaxed_lithiated_structure",
            valid_type=orm.StructureData,
            required=False,
            help="Relaxed original structure",
        )
        ...
        spec.output(
            ...
            valid_type=orm.Dict,
            required=True,
            help="Summary of the result of delithiation.",
        )
        spec.output(
            "output_voltages",
            valid_type=orm.Dict,
            required=False,
            help="Voltages computed from delithiation.",
        )

        spec.exit_code(
            201,
            "ERROR_UNKNOWN_STRATEGY",
            message="Strategy to delithiate is not known.",
        )
        spec.exit_code(
            301,
            "ERROR_INITIAL_RELAX_FAILED",
            message="The initial relaxation is failed.",
        )
        spec.exit_code(
            401,
    ...
    def setup(self):
        ...
        if strategy not in self._allowed_strategies:
            self.report(
                f"Strategy: {strategy} is not valid, valid ones are: {self._allowed_strategies}"
            )
    ...
    def check_li_ref(self):
        ...
        if not check_li_ref_calc(
            incar.get("encut"), incar.get("gga"), self.inputs.li_ref_group_name
        ):
            raise RuntimeError(
                f"Li reference calculations do not exist for GGA:{gga}, encut: {encut}"
            )
    ...
    def verify_initial_relax(self):
        ...
        self.report(
            "Initial relaxation is a success, registerred the relaxed structure"
        )
    ...
    def delithiate(self):
        ...
        self.out_many(
            {
                f"delithiated_structures.structure_{i:04d}": structure
        ...
        for istruc, (frame, mask) in enumerate(zip(all_frames, all_masks)):
            ...
            if not inputs.metadata.get("label"):
                inputs.metadata.label = (
                    frame.get_formula("count") + f" DELI {istruc:02d}"
                )
            ...
            if not deli_magmom_mapping:
                self.report(
                    "WARNING: Empty mapping given for magmom - keeping the original"
                )
                ...
                if magmom:
                    ...
                    new_array = magarray[
                        mapping
                    ]  # Use the mapping to get a new list of MAGMOM
                ...
                magmom = []
                default = deli_magmom_mapping.get(
                    "default", 0.6
                )  # Default MAGMOM is 0.6
    ...
    def result(self):
        ...
        for workchain in self.ctx.workchains:
            ...
            if (
                not workchain.is_finished_ok
                and workchain.exit_status not in self.ALLOWED_OK_EXIT_CODES
            ):
                self.report(
                ...
                if workchain.exit_status in self.ALLOWED_OK_EXIT_CODES:
                    self.report(
                        "Relaxation {} ({}) finished with exit code: {}, but treated as if it was OK".format(
    ...
    def get_voltages(self):
        ...
        if not relax:
            ...
            if lithiated_misc:
                ...
                relax = q.one()[
                    0
                ]  # Use this calcjob node as the lithiated calculation.
            else:
                self.report(
                    "Initial relaxation not performed - skipping voltage computation"
                )
                return None
        ...
        for workchain in self.ctx.workchains:
            if not workchain.is_finished_ok:
                self.report(
                    f"Skip failed workchain {workchain} for voltage calculation"
                )
                continue
            try:
                voltage = compute_li_voltage_shortcut(
                    relax,
                    workchain,
                    li_ref_group_name=self.inputs.li_ref_group_name,
                    store_provenance=False,
                )
            except RuntimeError:
                self.report(
                    "Cannot compute voltage - possibly the Li reference is not available"
                )
                break
            link_name = (
                workchain.get_incoming(link_type=LinkType.CALL_WORK).one().link_label
            ...
            voltage_data.append(
                {"relax_work": workchain.uuid, "relax_id": ist, "voltage": voltage}
            )
