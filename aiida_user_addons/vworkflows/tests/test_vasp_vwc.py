...
@pytest.mark.parametrize(
    ["vasp_structure", "vasp_kpoints"], [("str", "mesh")], indirect=True
)
def test_vasp_wc(
    fresh_aiida_env, vasp_params, potentials, vasp_kpoints, vasp_structure, mock_vasp
):
    ...
    workchain = WorkflowFactory("vaspu.vasp")
...
def test_vasp_wc_chgcar(
    ...
    inputs.settings = get_data_node(
        ...
        dict={
            ...
            "parser_settings": {"add_chgcar": True},
        },
    )
    inputs.max_iterations = get_data_node("int", 1)
    inputs.clean_workdir = get_data_node("bool", False)
...
def test_vasp_wc_ionic_continue(
    fresh_aiida_env, potentials, mock_vasp_strict, incar, nkpts, exit_codes
):
