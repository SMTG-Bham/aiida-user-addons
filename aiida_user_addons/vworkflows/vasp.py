...
from aiida_vasp.workchains.vasp import (
    VaspWorkChain as VanillaVaspWorkChain,
)
...
class VaspWorkChain(VanillaVaspWorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input(
            "chgcar", valid_type=get_data_class("vasp.chargedensity"), required=False
        )
        spec.input(
            "site_magnetization",
            valid_type=get_data_class("dict"),
    ...
    felec: Wether we are dealing with f electrons, will increase lmaxmix if we are.""",
        ...
        spec.output(
            "parallel_settings", valid_type=get_data_class("dict"), required=False
        )
    ...
    def init_inputs(self):
        ...
        if self.inputs.get("settings"):
            ...
            unsupported_parameters = settings_dict.get(
                "unsupported_parameters", unsupported_parameters
            )
            skip_parameters_validation = settings_dict.get(
        ...
        try:
            ...
            return self.exit_codes.ERROR_IN_PARAMETER_MASSAGER.format(
                exception=exception
        ...
        if "site_magnetization" in self.inputs:
            magmom = site_magnetization_to_magmom(
                self.inputs.site_magnetization.get_dict()
            )
        ...
        if not self.inputs.potential_family.value:
            ...
            return (
                self.exit_codes.ERROR_NO_POTENTIAL_FAMILY_NAME
            )  # pylint: disable=no-member
        try:
            self.ctx.inputs.potential = get_data_class(
                "vasp.potcar"
            ).get_potcars_from_structure(
            ...
            return compose_exit_code(
                self.exit_codes.ERROR_POTENTIAL_VALUE_ERROR.status, str(err)
            )  # pylint: disable=no-member
        except NotExistent as err:
            return compose_exit_code(
                self.exit_codes.ERROR_POTENTIAL_DO_NOT_EXIST.status, str(err)
            )  # pylint: disable=no-member
    ...
    def run_auto_parallel(self):
        ...
        return (
            "auto_parallel" in self.inputs and self.inputs.auto_parallel.value is True
        )
    ...
    def perform_autoparallel(self):
        ...
        self.report(
            f"Performing local dryrun for auto-parallelisation"
        )  # pylint: disable=not-callable
        ...
        try:
            ...
            self.report(
                f"Dry-run errorred, process with cautions, message: {error.args}"
            )  # pylint: disable=not-callable
            return

        if (scheme.ncore is None) or (scheme.kpar is None):
            self.report(
                f"Error NCORE: {scheme.ncore}, KPAR: {scheme.kpar}"
            )  # pylint: disable=not-callable
            return

        parallel_opts = {"ncore": scheme.ncore, "kpar": scheme.kpar}
        self.report(
            f"Found optimum KPAR={scheme.kpar}, NCORE={scheme.ncore}"
        )  # pylint: disable=not-callable
