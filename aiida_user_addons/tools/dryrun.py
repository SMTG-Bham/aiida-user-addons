...
from aiida_user_addons.cmdline.cmd_vasp_dryrun import (
    vasp_dryrun as _vasp_dryrun,
)
...
def dryrun_vasp(input_dict, vasp_exe="vasp_std", timeout=10, work_dir=None, keep=False):
    ...
    outcome = _vasp_dryrun(
        folder, vasp_exe=vasp_exe, timeout=timeout, work_dir=work_dir, keep=keep
    )
    if not keep:
...
def dryrun_relax_builder(builder, **kwargs):
    ...
    if builder.vasp.kpoints is not None:
        ...
        vasp_builder.kpoints.set_kpoints_mesh_from_density(
            builder.vasp.kpoints_spacing.value * np.pi * 2
        )
    vasp_builder.metadata.options = (
        builder.vasp.options.get_dict()
...
def dryrun_vaspu_builder(builder, **kwargs):
    ...
    if builder.kpoints is not None:
        ...
        vasp_builder.kpoints.set_cell_from_structure(builder.structure)
        vasp_builder.kpoints.set_kpoints_mesh_from_density(
            builder.kpoints_spacing.value * np.pi * 2
        )
    vasp_builder.metadata.options = (
        builder.options.get_dict()
    )  # pylint: disable=no-member
    vasp_builder.potential = PotcarData.get_potcars_from_structure(
        builder.structure,
