...
def dbtoaiida(db_path: Union[str, Path],
    ...
    with connect(db_path) as conn:
        for i, row in enumerate(conn.select(converged=False, struct_type='initial', **row_filters)):
            q = orm.QueryBuilder()
            q.append(orm.StructureData, filters={'attributes.ce_uuid': row.unique_id, 'attributes.ce_uid': row.id})
            if limit_to_group:
                q.append(orm.Group, filters={'id': group_structure.id}, with_node=orm.StructureData)
            if q.count() > 0:
...
def aiidatodb(db_path: Union[Path, str], group: orm.Group, calc_finder: Callable = None, reset_converge_tag=False):
    """
    ...
    for row in rows:
        ...
        custom_kvp_init = {'aiida_structure_uuid': init_it, 'aiida_relax_uuid': node.uuid}  # For the initial structure row
        custom_kvp_final = {'aiida_structure_uuid': ra.output_structure.uuid, 'aiida_relax_uuid': node.uuid}  # For the final structure row
...
def get_calc_finder(allowed_exit_status=0):
    ...
    def _inner(group_workflows, ce_initial_uuid) -> Tuple[orm.WorkChainNode, str]:
        ...
        q.append(orm.Node, with_group=orm.Group, filters={'attributes.exit_status': {'in': allowed_exit_status}}, project=['*'])
        q.append(orm.StructureData, with_outgoing=orm.Node, tag='reduced')
        q.append(orm.CalcFunctionNode, with_outgoing='reduced', filters={'attributes.function_name': 'niggli_reduce'})
        q.append(orm.StructureData,
