"""
Modified Ewald summation based on pymatgen.analysis.ewald

Changes:

- Replace the slow `sum(sum(X))` code patterns with `X.sum(axis=y)`
- Added the method for quick recomputing the energy upon a single change of the site charge
`
"""
import bisect
...
class EwaldSummation(MSONable):
    ...
    def __init__(
        ...
        self._forces = None
        self._total_energy_matrix = None
    ...
    def total_energy_matrix(self):
        ...
        if not self._initialized:
            self._calc_ewald_terms()
            self._initialized = True
        if self._total_energy_matrix is None:
            totalenergy = self._recip + self._real
            self._total_energy_matrix = totalenergy
        else:
            # Inplace operation
            totalenergy = self._total_energy_matrix
            totalenergy.fill(0.0)
            totalenergy += self._recip
            totalenergy += self._real
    ...
    def _calc_recip(self):
        ...
        erecip = np.zeros((numsites, numsites), dtype=float)
        forces = np.zeros((numsites, 3), dtype=float)
        coords = self._coords
    ...
    def _calc_real_and_point(self):
        ...
        coords = self._coords
        numsites = self._s.num_sites
        ereal = np.empty((numsites, numsites), dtype=float)
...
def compute_average_oxidation_state(site):
    ...
    try:
        ...
        raise ValueError(
            "Ewald summation can only be performed on structures " "that are either oxidation state decorated or have " "site charges."
        )


class EwaldSummationWithSwap:
    """Class for computing ewald energy with site swaps"""

    def __init__(self, summation: EwaldSummation):
        """Instantiate the object with an existing EwaldSummation"""

        self._charges = summation._charges.copy()
        self._summation = summation
        assert not summation._charged, "Charged unit cell is not supported"
        self.total_energy_matrix = summation.total_energy_matrix
        # Record the initial total energy
        self.total_energy = self.total_energy_matrix.sum()

    @property
    def is_charge_neutral(self):
        """Wether the system is charge netural"""
        return self._charges.sum() == 0

    def update_site_charge(self, idx, charge):
        """Update the charge of the site"""
        old_charge = self._charges[idx]
        new_charge = charge
        scale = new_charge / old_charge
        old_sum = self.total_energy_matrix[:, idx].sum() + self.total_energy_matrix[idx, :].sum()

        # Update the total_energy_matrix
        self.total_energy_matrix[:, idx] *= scale
        self.total_energy_matrix[idx, :] *= scale
        # Update the charges
        self._charges[idx] = charge

        # Compute the change of the total energy
        de = old_sum * (scale - 1)
        self.total_energy += de
        return self.total_energy

    def swap_sites(self, idx1, idx2):
        """Swap two sites and calculate the new total energy"""
        charge1 = self._charges[idx1]
        charge2 = self._charges[idx2]
        # Update the charge - this changes the total energy matrix
        self.update_site_charge(idx1, charge2)
        self.update_site_charge(idx2, charge1)
        return self.total_energy

    def _assert_consistency(self, tol=1e-10):
        """Check that the total energy is indeed the sum of the energy matrix"""
        assert abs(self.total_energy - self.total_energy_matrix.sum()) < tol
