...
class BetterMplPlotter(PDPlotter):
    ...
    def _get_2d_plot(
        ...
        if len(self._pd.elements) == 3:
            ...
            if (
                isinstance(self._pd, CompoundPhaseDiagram)
                and self._pd.normalize_terminals is False
            ):
        ...
        if self.show_unstable:
            ...
            for ientry, (entry, coords) in enumerate(unstable.items()):
                ...
                if ehull < self.show_unstable:
                    ...
                    if no_polyanion:
                        label = reduce_formula_no_polyanion(
                            Composition(entry.name).as_dict()
                        )[0]
                    else:
                        label = entry.name
...
class PrettyPlotlyPlotter(PDPlotter):
    ...
    def get_plot(
        ...
        if self.backend == "plotly":
            ...
            for i, entry_type in enumerate(self.unique_types):
                stable_labels_plot = self._create_plotly_stable_labels(
                    label_stable, entry_type=entry_type
                )
    ...
    def entry_types(self):
        ...
        return [
            entry.parameters.get(self.ENTRY_TYPE_STRING)
    ...
    def _create_plotly_stable_labels(self, label_stable=True, entry_type=None):
        ...
        name = (
            ...
            else f"Labels (stable) - {entry_type}"
        )
        plot_args = dict(
            text=text,
    ...
    def _create_plotly_markers(
        self, label_uncertainties=False, entry_type=None, colorbar=True
    ):
        """
        ...
        def get_marker_props(coords, entries, stable=True):
            ...
            for coord, entry in zip(coords, entries):
                ...
                label += (
                    f"<br> entry_type: {entry.parameters.get(self.ENTRY_TYPE_STRING)}"
                )
        ...
        stable_coords, stable_entries = (
            [
                key
                for key, value in self.pd_plot_data[1].items()
                if value.parameters.get(self.ENTRY_TYPE_STRING) == entry_type
            ],
            [
                value
                for key, value in self.pd_plot_data[1].items()
        ...
        unstable_entries, unstable_coords = (
            [
                key
                for key, value in self.pd_plot_data[2].items()
                if key.parameters.get(self.ENTRY_TYPE_STRING) == entry_type
            ],
            [
                value
                for key, value in self.pd_plot_data[2].items()
        ...
        stable_marker_props = self.get_stable_marker(
            entry_type, self._dim, stable_props
        )

        unstable_name = (
            "Above hull" if entry_type is None else f"Above hull - {entry_type}"
        )
        unstable_marker_props = self.get_unstable_marker(
            entry_type, self._dim, unstable_props, colorbar
        )
