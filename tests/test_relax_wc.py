...
def data_path(name=""):
    ...
    return Path(__file__).parent / "../workflow_test_data" / name
...
@pytest.mark.skip("Support data rebuilt is required")
def test_relax_wc_continue(fresh_aiida_env, potentials, mock_vasp_strict):
    ...
    inputs = setup_relax_workchain(si_structure(), INCAR_RELAX_CONTINUE, OPT_RELAX_CONTINUE, 8)
